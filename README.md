...
- **First task appears slow:** allow the checkpoint download to finish and check free disk space. Repeated decisions do not redownload it. Set `OPENULTRA_MODEL_PATH` to an existing compatible checkpoint to use a local copy.
...
OpenUltra uses a typed decision model rather than asking a language model to generate selectors or scripts. A typed decision asks a fixed question such as *which observed operation?*, *which visible target?*, or *is the goal complete?* and returns probabilities over declared options. This is the same decision interface explored by TypeSafe's **Jev** work. **Laya** is the open-weight decision model used here, executed locally through `laya-mlx`. Neither model writes arbitrary form text: OpenUltra binds only literal values supplied in the task or named prepared inputs.

```text
task + visible page
      -> one bounded DOM observation and indexed action space
      -> one local Laya batch: operation, compatible targets, completion, stuck
...
The browser adapter performs one page observation per cycle and offers only actions grounded in visible controls. Batching the compatible decision questions avoids a model call for each possible button. Browser work, page loading, speech recognition, and model loading still contribute to total task time; decision latency alone is not end-to-end speed. OpenUltra shows elapsed time, latest model decision time, and decisions per second in the inspector.
...
- This is an **alpha** browser agent, not a claim of reliable completion for arbitrary sites or tasks. Generic Laya weights are not browser-specialized, and probabilities are not guarantees of correctness.
...
The decision-model explanation draws on [TypeSafe's Jev introduction](https://typesafe.ai/blog/introducing-system-one-models-and-jev), [Laya](https://github.com/NandhaKishorM/laya), [Laya MLX](https://github.com/mizorewww/laya-mlx), and [Laya's typed Node interface](https://github.com/receptron/laya). Browser-agent implementation references include [Jev Ultrafast](https://github.com/browser-use/jev-ultrafast), [Jev Browser](https://github.com/Ying-Kai-Liao/jev-browser), and [Jev Voice Browser](https://github.com/moritzkremb/jev-voice-browser). OpenUltra is independent of these projects and does not use Jev or a TypeSafe service at runtime.
